...
def generateData():
    ...
    x = x.reshape((batch_size, -1)) # the first index changing slowest, subsieries as rows
...
# vizualizing
def plot(loss_list, prediction_series, X, Y):
    plt.subplot(2, 3, 1)
    plt.cla()
    plt.plot(loss_list)

    for batch_series_idx in range(5):
        one_hot_ouput_series = np.array(prediction_series)[:, batch_series_idx, :]
        single_output_series = np.array([(1 if out[0] < 0.5 else 0) \
                                         for out in one_hot_ouput_series])
        plt.subplot(2, 3, batch_series_idx + 2)
        plt.cla()
        plt.axis([0, truncated_backprop_length, 0, 2])
        left_offset = range(truncated_backprop_length)
        plt.bar(left_offset, X[batch_series_idx, :], width=1, color="blue")
        plt.bar(left_offset, Y[batch_series_idx, :] * 0.5, width=1, color="red")
